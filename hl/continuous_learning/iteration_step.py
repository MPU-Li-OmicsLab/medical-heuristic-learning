...
from hl.agent.continuous_prompts import get_continuous_iteration_prompt
...
def run_iterations_task(
    *,
    ...
    log_progress("HL-CL-ITER", f"Loading heuristic module from {heuristic_path}.")
    ...
    log_progress("HL-CL-ITER", f"Baseline v0 metrics: {metrics_v0}.")

    if not run_cfg.run_iterations:
        log_progress("HL-CL-ITER", "Iteration stage is disabled; returning v0 only.")
        return records, iteration_log
    if client is None:
        log_progress("HL-CL-ITER", "LLM client is unavailable; skipping iterative optimization.")
        return records, iteration_log

    for i in range(1, max(0, run_cfg.iterations) + 1):
        next_version = f"v{i}"
        log_progress("HL-CL-ITER", f"Starting iteration {i}/{max(0, run_cfg.iterations)} for {next_version}.")
        ...
        prompt = get_continuous_iteration_prompt(
        ...
        for attempt in range(1, max(1, run_cfg.max_llm_attempts) + 1):
            ...
            try:
                ...
                attempt_logs.append({"attempt": attempt, "status": "json_parse_failed", "error": str(exc)})
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: json_parse_failed ({exc}).")
                continue
            ...
            if proposal.version != next_version:
                ...
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: version_mismatch.")
                continue
            ...
            try:
                ...
                attempt_logs.append({"attempt": attempt, "status": "syntax_invalid", "error": str(exc)})
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: syntax_invalid ({exc}).")
                continue
            except Exception as exc:
                attempt_logs.append({"attempt": attempt, "status": "undefined_name", "error": str(exc)})
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: undefined_name ({exc}).")
                continue
            ...
            if fn_name != f"predict_{next_version}":
                ...
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: function_name_mismatch.")
                continue
            ...
            if current_fn is None:
                ...
                log_progress("HL-CL-ITER", f"{next_version} attempt {attempt} failed: load_failed.")
                continue
            ...
            degradation = detect_degradation(y_true_train, y_pred_train, y_pred_train_new)
            examples = collect_degradation_examples(
                df=train_df,
                label_col=label_col,
                degraded_indices=degradation.degraded_indices,
            ...
            last_regressed_indices = list(degradation.degraded_indices)
        ...
        if not accepted:
            log_progress("HL-CL-ITER", f"Stopping iterations because {next_version} was not accepted.")
            break

    log_progress("HL-CL-ITER", f"Iteration stage finished with {len(records)} recorded versions.")
    return records, iteration_log
