...
def _sample_train_balanced(
    ...
    *,
    ...
    meta = {
        "train_pos_target": int(pos_each),
        "train_neg_target": int(neg_each),
        "train_pos_available": int(len(pos_pool)),
        "train_neg_available": int(len(neg_pool)),
        "train_pos_replace": bool(pos_replace),
        "train_neg_replace": bool(neg_replace),
    }
...
def _run_one(
    *,
    ...
    print(f"[ablation_balance] dataset={dataset_name} ablation={cfg_name} train={train_size} out_dir={out_dir}", flush=True)
    if bool(train_meta.get("train_pos_replace")) or bool(train_meta.get("train_neg_replace")):
        print(f"[ablation_balance] resample_with_replacement meta={train_meta}", flush=True)
    ...
    run_cfg = RunConfig(
        ...
        task_description=(
            ...
            "Training set is 1:1 balanced by sampling."
        ),
    )
    llm_cfg = LLMConfig(
    ...
    print(
        f"[ablation_balance] done dataset={dataset_name} ablation={cfg_name} train={train_size} final_version={final_version} out_dir={out_dir}",
        flush=True,
    )
    return out_dir
...
def main() -> None:
    ...
    if workers <= 1:
        ...
        if workers > (os.cpu_count() or 1) * 4:
            print(f"[ablation_balance] warning: workers={workers} is very high; you may hit API rate limits.", flush=True)
        results: list[dict] = []
        with ProcessPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in as_completed(fut_map):
                ...
                if r.get("status") == "ok":
                    print(
                        f"[ablation_balance] finished dataset={r['dataset']} ablation={r['ablation']} train={r['train_size']} out_dir={r['out_dir']}",
                        flush=True,
                    )
                else:
                    print(
                        f"[ablation_balance] failed dataset={r['dataset']} ablation={r['ablation']} train={r['train_size']} error={r.get('error','')}",
                        flush=True,
                    )
    ...
    print(f"[ablation_balance] index_written={index_path}", flush=True)
    ...
    print(f"[ablation_balance] csv_written={csv_path}", flush=True)
