...
def _run_cpu_models_block(
    ...
    train_df = _sample_train_random(train_pool, train_size=train_size, seed=seed + train_size)
    ...
    for model_name, estimator in models:
        try:
            ...
            if model_name == "MLP" and "test_size" in str(e) and "number of classes" in str(e):
                try:
                    est2 = MLPClassifier(
                        hidden_layer_sizes=(256, 128),
                        activation="relu",
    ...
    class FTTransformer(torch.nn.Module):
        ...
        def forward(self, x_num, x_cat):
            ...
            if self.num_proj is not None and x_num is not None:
                z = self.num_proj(x_num)
                z = z.view(b, self.n_num, d_token)
                tokens.append(z)
...
def run_contrast(seed: int = 42, workers: int = 1) -> Path:
            ...
            for ds, train_size in cpu_jobs:
                futs.append(
                    ex.submit(
                        ...
                        int(seed),
                        int(train_size),
                        int(split_spec.val_total),
                        int(split_spec.test_total),
                    )
                )
        ...
        for train_size in train_sizes:
            ...
            try:
                rft = _fit_eval_ft_transformer(
                    ...
                    seed=seed,
                    checkpoint_dir=out_dir / "checkpoints" / ds.name / f"train{train_size}",
                )
                rows.append(
                    {
                        "模型": "FT-Transformer",
    ...
    out_path = out_dir / "contrast1.csv"
...
def main() -> None:
    ...
    args = p.parse_args()
    out = run_contrast(seed=int(args.seed), workers=int(args.workers))
    print(f"contrast1_csv={out}", flush=True)
